...
void sACNSentUniverse::setNetworkInterface(QNetworkInterface networkInterface)
{
...
void sACNSentUniverse::startSending(bool preview)
{
    ...
    CStreamServer *streamServer = CStreamServer::getInstance(m_networkInterface);
    if(m_cid.isNull())
        m_cid = CID::CreateCid();
    ...
    uint max_tx_rate = E1_11::MAX_REFRESH_RATE_HZ;
...
{
    CStreamServer::getInstance(m_networkInterface)->DestroyUniverse(m_handle);
    m_handle = 0;
    m_isSending = false;

    if(m_priorityMode == pmPER_ADDRESS_PRIORITY)
    {
        CStreamServer::getInstance(m_networkInterface)->DestroyUniverse(m_priorityHandle);
        m_priorityHandle = 0;
    }
}
...
void sACNSentUniverse::setName(const QString &name)
{
    ...
    if(isSending())
    {
        ...
        CStreamServer::getInstance(m_networkInterface)->setUniverseName(m_handle ,arr.constData());
    }
}
...
void sACNSentUniverse::setPerSourcePriority(quint8 priority)
{
    ...
    if(m_isSending)
    {
         CStreamServer::getInstance(m_networkInterface)->setUniversePriority(m_handle, priority);
    }
}
...
void sACNSentUniverse::setSynchronization(quint16 address)
{
    ...
    if(m_isSending)
    {
         CStreamServer::getInstance(m_networkInterface)->setSynchronizationAddress(m_handle, m_synchronization);
    }
...
CStreamServer::CStreamServer(QNetworkInterface t_interface) :
    m_thread_stop(false),m_interface(t_interface)
{
    m_sendsock = new sACNTxSocket(m_interface);
