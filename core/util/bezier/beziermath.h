...
namespace BezierMath {
    ...
    static double arcfn(double t_t, std::function<point_d(double)> t_derivFunc)
    {
        const point_d d = t_derivFunc(t_t);
    ...
    static double length(std::function<point_d(double)> t_derivFunc)
    {
        const double z = 0.5;
        size_t len = 24;
        ...
        for (size_t i = 0, t; i < len; i++) {
          t = z * Tvalues[i] + z;
          sum += Cvalues[i] * arcfn(t, t_derivFunc);
        }
        return z * sum;
    }
    ...
    using BezierPair = std::pair<Bezier,Bezier>;
    using DoublePair = std::pair<double, double>;

    static std::vector<DoublePair> pairiteration(const Bezier &c1, const Bezier &c2, double curveIntersectionThreshold = .5)
    {
        auto c1b = c1.bounds();
        auto c2b = c2.bounds();
        double r = 100000;
        double threshold = curveIntersectionThreshold;

        //qDebug() << "test" << c1b << c2b;
        if (c1b.width() + c1b.height() < threshold && c2b.width() + c2b.height() < threshold) {
          return {DoublePair(floor((r * (c1.t1() + c1.t2())) / 2) / r, floor((r * (c2.t1() + c2.t2())) / 2) / r)};
          //return {DoublePair(.5,c2.t1())};
        }
        ...
        std::vector<BezierPair> pairs = {BezierPair(cc1.left, cc2.left),
                                        BezierPair(cc1.left, cc2.right),
                                        BezierPair(cc1.right, cc2.right),
                                        BezierPair(cc1.right, cc2.left)};

        pairs.erase(std::remove_if(pairs.begin(), pairs.end(),[](const BezierPair &pair){
            //return !overlaps(pair.first.bbox(),pair.second.bbox());
            return !pair.first.bounds().intersects(pair.second.bounds());
        }), pairs.end());

        std::vector<DoublePair> results;

        if(pairs.empty())
            return results;

        std::for_each(pairs.begin(), pairs.end(),[&results](const BezierPair &pair){
            auto r = pairiteration(pair.first, pair.second);
