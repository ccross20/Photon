...
const Vector3 Line::pointOnLine(Vector3 ptA, Vector3 ptB, float t_position)
{
    Vector3 result;
    double dx = ptB.x-ptA.x;
    double dy = ptB.y-ptA.y;
    double dz = ptB.z-ptA.z;
    result.x = dx*t_position + ptA.x;
    result.y = dy*t_position + ptA.y;
    result.z = dz*t_position + ptA.z;
